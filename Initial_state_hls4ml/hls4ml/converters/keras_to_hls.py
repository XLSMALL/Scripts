...
def keras_to_hls(config):
    ...
    for keras_layer in layer_config:
        ...
        if 'inbound_nodes' in keras_layer and len(keras_layer['inbound_nodes']) > 0:
            # print("inbound_nodes[0]", keras_layer['inbound_nodes'][0]) ## need to comment out
            # print("inbound_nodes[0][0][-1]", keras_layer['inbound_nodes'][0][0][-1])
            input_names = [ inputs_map.get(inp[0], inp[0]) for inp in keras_layer['inbound_nodes'][0] ]
            # for inp in keras_layer['inbound_nodes'][0]:
            #     print("map get", inputs_map.get(inp[0], inp[0]))
            # print("input_names", input_names)
            # print("keras_layer", keras_layer)
            if keras_layer['inbound_nodes'][0][0][-1] and keras_layer['class_name'] == 'GRU':
