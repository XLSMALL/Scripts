  ...
  if (fin.is_open() && fpr.is_open()) {
    while ( std::getline(fin,iline) && std::getline (fpr,pline) ) {
      ...
      if (e % CHECKPOINT == 0) {
        ...
        for(int i = 0; i < N_OUTPUTS_13*N_FILT_13; i++) {
          std::cout << pr[i] << " ";
        }
        std::cout << std::endl;
        std::cout << "Quantized predictions" << std::endl;
        //hls-fpga-machine-learning insert quantized
        nnet::print_result<result_t, N_OUTPUTS_13*N_FILT_13>(layer13_out, std::cout, true);
      }
      e++;

      //hls-fpga-machine-learning insert tb-output
      nnet::print_result<result_t, N_OUTPUTS_13*N_FILT_13>(layer13_out, fout);
